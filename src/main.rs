...
// TODO: read nameでソートして，read nameが変わったら，出力するというコードに変える必要あり。
...
fn cal_count_marker(bamfile: &str, hap1_tabix: &str, hap2_tabix: &str, kmer_size: u32) ->  HashMap<String, Vec<(String, i64, i64, u32, u32, String, usize, usize, usize)>> {
    ...
    //let sec_alignments: HashMap<String, Vec<String>> = HashMap::new();
    ...
    for record in bam.records() {
        ...
        if record.is_reverse() {
            if record.is_supplementary() | record.is_secondary() {
                if let Some(value) = sequences.get(&read_id) {
                    read_seq = reverse_complement(&value.to_owned());
                } else {
                    eprintln!("Bad alignment: BamFile needs to be sorted");
                }
            } else {
                read_seq = reverse_complement(&convert_u82String(&record.seq().as_bytes()));
            }
        } else {
            if record.is_supplementary() | record.is_secondary() {
                if let Some(value) = sequences.get(&read_id) {
                    read_seq = value.to_owned();
                } else {
                    eprintln!("Bad alignment: BamFile needs to be sorted");
                }
            } else {
                read_seq = convert_u82String(&record.seq().as_bytes());
            }
        }

        if !record.is_secondary() & !record.is_supplementary() {
            if record.is_reverse() {
                let reverse_sequence = reverse_complement(&read_seq);
                sequences.insert(read_id.clone(), reverse_sequence.clone());
            } else {
                sequences.insert(read_id.clone(), read_seq.clone());
            }
        }
        ...
        if &reference_name[0..2] == "h1" {
            let tid = match tbx_reader.tid(reference_name) {
                ...
                Err(_) => {
                    ...
                    alignments.entry(read_id).or_insert(Vec::new()).push(info);
                    continue;
                },
            };
            ...
            tbx_reader = &mut hap2_tbx_reader;
            let tid = match tbx_reader.tid(reference_name) {
                Ok(tid) => tid,
                Err(_) => {
                    let info: (String, i64, i64, u32, u32, String, usize, usize, usize) 
        ...
        // eprintln!("Iterator: {} {}", it_start, it_end);
