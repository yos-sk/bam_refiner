...
pub fn run(alignments:Vec<Data>, indices: HashMap<String, Vec<usize>>) -> Result<HashMap<String, Vec<Data>>, Box<dyn Error>> {
    ...
    for (read_id, index) in indices.iter() {
        ...
        for i in index.iter() {
            assert_eq!(alignments[*i].read_name, read_id.to_string(), "Read names are inconsistent: {} {}", alignments[*i].read_name, read_id);
        ...
        assert!(!primary.is_empty(), "Read: {} doesn't have any primary alignment", read_id);
        ...
        for t_secondary in secondary.iter_mut() {
            ...
            if !supplementary.is_empty() {
                for (i, t_supplementary) in supplementary.iter().enumerate() {
                    let s_diff_start = t_supplementary.read_start as isize - t_secondary.read_start as isize;
                    let s_diff_end = t_supplementary.read_end as isize - t_secondary.read_end as isize;
